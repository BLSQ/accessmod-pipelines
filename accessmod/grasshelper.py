...
class GrassError(Exception):
    """Error when running GRASS GIS command."""

    pass


class GrassNotFoundError(Exception):
    """GRASS GIS not found."""
...
def check_mapset(gisdb_path, location_name, mapset_name):
    ...
    if os.path.exists(permanent_path):
        ...
        if not os.path.exists(wind_path):
            logger.error("PERMANENT mapset already exists, but a WIND file is missing.")
        else:
